...
public class UserController {
    ...
    public Result<?> Login(@RequestBody User user) {
        ...
        if (loginRole.getId() != 3) {
            ...
            // 一级菜单
            List<Menu> parentList = roleMenuList.stream().filter(menu -> menu.getPid() == null)
                    .collect(Collectors.toList());
            for (Menu parentMenu : parentList) {
                List<Menu> childrenList = roleMenuList.stream().filter(menu -> parentMenu.getId().equals(menu.getPid()))
                        .collect(Collectors.toList());
    ...
    public Result<?> getUsersByPage(
        ...
        LOGGER.info("username:" + username + " sex:" + sex + " name" + name + "role" + role + " currentRole:"
                + currentRole + " currentPage:" + currentPage + " size:" + size);
        LambdaQueryWrapper<User> queryWrapper = Wrappers.lambdaQuery();
    ...
    public Result<?> createUser(@RequestBody User user) {
        // 检查用户名是否已存在
        Long checkUsername = userMapper
                .selectCount(new LambdaQueryWrapper<User>().eq(User::getUsername, user.getUsername()));
        ...
        if (!StringUtils.isNotBlank(user.getRole())) {
            ...
            if (!Arrays.asList(UserRole.ADMIN.getValue(), UserRole.USER.getValue(), UserRole.SUPER_ADMIN.getValue())
